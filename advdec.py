...
def upload_to_google_sheets(sheet_id, tab_name, dataframe, credentials_path):
...
def save_data_to_google_sheets_and_csv():
    ...
    credentials_path = "C:/Users/user/Downloads/Compressed/1.unofficial/Credentials.json"
    ...
    if most_active_data is not None and not most_active_data.empty:
        ...
        upload_to_google_sheets(sheet_id, "Most Active", most_active_df, credentials_path)
        # Save to CSV
...
sheet = client.open_by_key(sheet_id)
try:
    worksheet = sheet.worksheet("Adv_Dec")  # Use an existing tab
except gspread.exceptions.WorksheetNotFound:
    worksheet = sheet.add_worksheet(title="Adv_Dec", rows="100", cols="25")  # Create a new tab
...
print("Data successfully saved to Google Sheet in tab 'Adv_Dec'!")
